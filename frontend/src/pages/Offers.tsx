...
const StatusBadge = ({ statusText, type }) => {
  ...
  return (
    <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-medium ${getStatusColor()}`}>
      {getStatusIcon()}
      {statusText}
    </span>
  );
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {[...Array(3)].map((_, index) => (
                <motion.div
                  key={index}
                  className="w-[300px] h-[300px] bg-[#1E1A2B] rounded-2xl animate-pulse"
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-col gap-1 text-base text-[#B0B0B0]">
                      <div className="flex items-center gap-2">
                        <Building className="h-4 w-4 text-[#F700FC]" />
                        <span className="line-clamp-1">{offer.companyName || "N/A"}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-[#F700FC]" />
                        <span className="line-clamp-1">
                          {offer.date ? new Date(offer.date).toDateString() : "N/A"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4 text-[#F700FC]" />
                        <span className="text-[14px] line-clamp-1">{offer.description}</span>
                      </div>
                    </div>
